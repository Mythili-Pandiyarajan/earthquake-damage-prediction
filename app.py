...
CAT_LABELS = {
    "land_surface_condition": {
        "n": "n · Normal flat ground",
        "o": "o · Oblique hillside",
        "t": "t · Terraced hillside",
    },
    "foundation_type": {
        "h": "h · Bamboo / timber",
        "i": "i · Mud / earthen",
        "r": "r · Stone / brick  (most common)",
        "u": "u · Cement / RC  (strongest)",
        "w": "w · Other",
    },
    "roof_type": {
        "n": "n · Bamboo / timber  (highest risk)",
        "q": "q · RCC / concrete  (strongest)",
        "x": "x · Light metal / other",
    },
    "ground_floor_type": {
        "f": "f · Mud  (weakest)",
        "m": "m · Brick / stone",
    ...
    "other_floor_type": {
        "j": "j · Timber / bamboo  (weakest)",
        "q": "q · RCC  (strongest)",
        "s": "s · Other",
        "x": "x · Not applicable",
    },
    "position": {
        "j": "j · Stand-alone (not attached)",
        "o": "o · Attached on 1 side",
        "s": "s · Attached on 2 sides",
        "t": "t · Attached on 3 sides",
    },
    "plan_configuration": {
        "a": "a · Irregular shape",
        "c": "c · C-shape",
        "d": "d · Rectangular  (most common)",
        "f": "f · F-shape",
        "m": "m · Multi-edge",
        "n": "n · N-shape",
        "o": "o · Circular",
    ...
    "legal_ownership_status": {
        "a": "a · Public / government",
        "r": "r · Institutional",
        "v": "v · Private  (most common)",
        "w": "w · Other",
    },
}
CAT_OPTIONS = {k: list(v.keys()) for k, v in CAT_LABELS.items()}

CAT_HELP = {
    "land_surface_condition": "Terrain type where the building sits. Terraced hillsides tend to be more vulnerable.",
    "foundation_type":        "Foundation material. Stone/brick (r) is most common but vulnerable. RC (u) is strongest.",
    "roof_type":              "Roof material. Bamboo/timber (n) is most common and shows highest damage in data. RCC (q) performs best.",
    "ground_floor_type":      "Ground floor material. Mud (f) is weakest; RC (x) is strongest.",
    "other_floor_type":       "Upper floor material. Timber/bamboo (j) is weakest; RCC (q) is strongest.",
    "position":               "Whether the building is attached to neighbours. Attached buildings can transfer stress during quakes.",
    "plan_configuration":     "Shape of the building footprint from above. Rectangular (d) is most common in Nepal.",
    "legal_ownership_status": "Ownership type. Affects construction standards and maintenance quality.",
}
...
with st.sidebar:
    ...
    <div style='font-size:0.72rem;color:#374151;line-height:1.9'>
        <b style='color:#4b5563'>Quick Code Guide</b><br>
        <b style='color:#6b7280'>Roof</b> n=bamboo · q=RCC · x=metal<br>
        <b style='color:#6b7280'>Foundation</b> r=stone · u=RC · i=mud<br>
        <b style='color:#6b7280'>Ground floor</b> f=mud · x=RC · m=brick<br>
        <b style='color:#6b7280'>Position</b> j=alone · s=2sides · t=3sides<br>
        <b style='color:#6b7280'>Plan</b> d=rect · s=square · u=L · q=T<br>
        <b style='color:#6b7280'>Ownership</b> v=private · a=public
    </div>
    """, unsafe_allow_html=True)
...
if page == "🔍 Predict":
    ...
    # ── How-to banner ─────────────────────────────────────────────────────────
    with st.expander("📖  How to use this form  —  click to expand", expanded=False):
        h1, h2, h3 = st.columns(3)
        with h1:
            st.markdown("""
            **🗺️ Geographic fields**
            Enter the **target-encoded mean damage score** for each geographic level (0–3).
            - `1.0–1.5` → historically low-damage region
            - `2.0` → average risk (default)
            - `2.5–3.0` → historically high-damage region

            These replace raw district/ward IDs from the original dataset.
            """)
        with h2:
            st.markdown("""
            **🏗️ Structural dropdowns**
            Each option shows the **code + plain English meaning**.
            Codes come from the original competition dataset where labels were hidden.

            Key risk signals:
            - Foundation `r` (stone/brick) → most vulnerable
            - Roof `n` (bamboo/timber) → highest damage rate
            - Foundation `u` (RC) → strongest protection
            """)
        with h3:
            st.markdown("""
            **🧱 Superstructure checkboxes**
            Check the materials used to build the main walls/structure.

            - ☑️ **Mud mortar stone** → highest damage risk
            - ☑️ **RC engineered** → best protection, check if present
            - Multiple materials can be selected

            **📐 Building basics**
            Higher age × more floors = greater vulnerability.
            """)
    ...
    with col_mid:
        ...
        def labeled_select(label, key):
            opts   = list(CAT_LABELS[key].values())
            chosen = st.selectbox(label, opts, help=CAT_HELP[key])
            return chosen[0]   # first char is the raw code

        land   = labeled_select("Land surface condition",  "land_surface_condition")
        found  = labeled_select("Foundation type",         "foundation_type")
        roof   = labeled_select("Roof type",               "roof_type")
        gfloor = labeled_select("Ground floor type",       "ground_floor_type")
        ofloor = labeled_select("Other floor type",        "other_floor_type")
        pos    = labeled_select("Position",                "position")
        plan   = labeled_select("Plan configuration",      "plan_configuration")
        legal  = labeled_select("Legal ownership status",  "legal_ownership_status")
